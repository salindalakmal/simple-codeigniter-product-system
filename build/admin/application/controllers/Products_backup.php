...
class Products extends MY_Controller {
	...
	public function upload() {
        ...
        if (isset($_FILES['pro_picture_tmp']['error']) && $_FILES['pro_picture_tmp']['error'] != 4) {

            $rand_val = date('YMDHIS') . rand(11111, 99999);
            $rand_num = md5($rand_val);

            // -------------------------------------
            $upload_path = $this->assets_path . 'profiles/temp';
            ...
            if ($this->upload->do_upload('pro_picture_tmp')) {
...
$pos  = strpos($uploade_image, ';');
$type = explode(':', substr($uploade_image, 0, $pos))[1];
...
echo $type;
					...
					$uploade_image = str_replace('data:image/png;base64,', '', $uploade_image);
					$uploade_image = str_replace(' ', '+', $uploade_image);

					$uploade_image = base64_decode($uploade_image);
					var_dump($uploade_image);
